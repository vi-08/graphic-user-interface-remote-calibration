        ...
        if HAS_ALLANTOOLS:
            try:
                tau_targets = np.unique(np.concatenate([
                    np.array([960.0, 1e3, 1e4, 1e5, 86400.0, 864000.0]),
                    np.logspace(np.log10(epoch_sec), np.log10(max(len(phase_s) * epoch_sec / 3, 864000.0)), 60)
                ]))
                tau_targets = tau_targets[tau_targets >= epoch_sec]
                ...
                method = 'allantools.mdev (phase data, explicit taus)'
